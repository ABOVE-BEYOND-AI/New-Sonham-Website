...
const ScrollExpandMedia = ({
  ...
  // Check if section is in viewport and user should interact with it
  useEffect(() => {
    const checkInView = () => {
      ...
      // Only consider section "active" when its top is near the viewport top
      // This prevents the effect from triggering too early
      const inView = rect.top <= 100 && rect.bottom > windowHeight * 0.5;
      setIsInView(inView);
    };
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      ...
      // Only handle scroll if we're at the top of the section
      if (!isAtTop && !mediaFullyExpanded) return;
      
      if (mediaFullyExpanded && e.deltaY < 0 && rect.top >= -50) {
        setMediaFullyExpanded(false);
        e.preventDefault();
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      // Only handle touch if we're at the top of the section
      if (!isAtTop && !mediaFullyExpanded) return;

      const touchY = e.touches[0].clientY;
      const deltaY = touchStartY - touchY;

      if (mediaFullyExpanded && deltaY < -20 && rect.top >= -50) {
        setMediaFullyExpanded(false);
        e.preventDefault();
      } else if (!mediaFullyExpanded && isAtTop) {
        e.preventDefault();
    ...
    const handleScroll = (e: Event): void => {
      // Don't interfere with normal scrolling when section is not active
      if (!isInView || mediaFullyExpanded) return;
      
      // When section is active but media not fully expanded,
      // we let the wheel/touch handlers manage the interaction
      // No need to force scroll position
    };
